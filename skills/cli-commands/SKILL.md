...
Watch local file changes and live-reload the dev page for preview. Does NOT deploy to the remote workspace — use orvanta sync push for that.

**Options:**
- `--includes <pattern...:string>` - Filter paths given a glob pattern or path
- `--proxy-port <port:number>` - Port for a localhost reverse proxy to the remote Orvanta server
...
Manage git-sync settings between local orvanta.yaml and Orvanta backend

**Subcommands:**

- `gitsync-settings pull` - Pull git-sync settings from Orvanta backend to local orvanta.yaml
...
- `gitsync-settings push` - Push git-sync settings from local orvanta.yaml to Orvanta backend
  - `--repository <repo:string>` - Specify repository path (e.g., u/user/repo)
...
Bootstrap a orvanta project with a orvanta.yaml file
...
Validate Orvanta flow, schedule, and trigger YAML files in a directory
...
- `sync pull` - Pull any remote changes and apply them locally.
  ...
  - `-e --excludes <patterns:file[]>` - Comma separated patterns to specify which file to NOT take into account. Overrides orvanta.yaml excludes
...
- `workspace bind` - Create or update a workspace entry in orvanta.yaml from the active profile
