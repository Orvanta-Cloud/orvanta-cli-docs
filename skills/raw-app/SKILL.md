...
Raw apps let you build custom frontends with React, Svelte, or Vue that connect to Orvanta backend runnables and datatables.

## Creating a Raw App

**You — the AI agent — create the app yourself by running `orvanta app new` with the right flags. Do NOT tell the user to "run `orvanta app new` and follow the prompts" or wait for them to do it.** The bare `orvanta app new` is an interactive wizard that hangs waiting for stdin in any non-TTY context (which includes you). Always pass flags.
...
2. **path** — the orvanta path, e.g. `f/folder/my_app` or `u/username/my_app`
...
| Flag | When to add it |
|---|---|
| `--datatable <name>` | The user wants this app wired to a specific Orvanta datatable. Without it, the app is created with no datatable. |
...
After `orvanta app new` and any initial edits to `App.tsx` / `index.tsx`, **offer** to open the visual preview as a one-sentence next step (e.g. "Want me to open the visual preview?"). Don't auto-open — opening the dev page has side effects (browser window, possibly a `launch.json` entry when an embedded preview tool is in play) the user should consent to.

For apps the preview command runs from the app folder (`cd <app_path>__raw_app && orvanta app dev …`); the `preview` skill picks the proxy vs direct branch based on whether the runtime exposes a tool that can embed a localhost URL. If the user already asked to see/preview/visualize the app in their original request, skip the offer and just invoke the skill.

### Anti-patterns to avoid

- ❌ Running `orvanta app new` with no flags (the prompt will hang).
- ❌ Telling the user to "run `orvanta app new` and follow the prompts" — that's a step backwards from what you can do directly.
...
├── DATATABLES.md          # Database schemas (run 'orvanta app generate-agents' to refresh)
├── raw_app.yaml           # App configuration (summary, path, data settings)
├── index.tsx              # Frontend entry point
├── App.tsx                # Main React/Svelte/Vue component
├── index.css              # Styles
├── package.json           # Frontend dependencies
├── orvanta.ts               # Auto-generated backend type definitions (DO NOT EDIT)
...
│   └── <id>.lock          # Lock file (run 'orvanta generate-metadata' to create/update)
...
To use an existing Orvanta script instead of inline code:
...
Import from the auto-generated `orvanta.ts`:

```typescript
import { backend } from './orvanta';

// Call a backend runnable
const user = await backend.get_user({ user_id: '123' });
```

The `orvanta.ts` file provides type-safe access to all backend runnables.

## Data Tables

Raw apps can query Orvanta datatables (PostgreSQL databases managed by Orvanta).
...
export async function main(user_id: string) {
  const sql = orvanta.datatable();  // Or: orvanta.datatable('other_datatable')
...
def main(user_id: str):
    db = orvanta.datatable()  # Or: orvanta.datatable('other_datatable')
...
2. Run `orvanta app dev` - the dev server watches this folder
...
`orvanta app new` is the exception: you run it yourself, with flags, per the "Creating a Raw App" section above.
...
| Command | Description |
|---------|-------------|
| `orvanta app dev` | Start dev server with live reload (see the `preview` skill for the full open-the-app-in-the-IDE-pane procedure). |
| `orvanta app generate-agents` | Refresh AGENTS.md and DATATABLES.md |
| `orvanta generate-metadata` | Generate lock files for backend runnables |
| `orvanta sync push` | Deploy app to Orvanta |
| `orvanta sync pull` | Pull latest from Orvanta |
...
6. **Generate locks** - tell the user to run `orvanta generate-metadata` after adding/modifying backend runnables
