...
- `orvanta script preview <script_path>` — **default when iterating on a local script.** Runs the local file without deploying.
- `orvanta script run <path>` — runs the script **already deployed** in the workspace. Use only when the user explicitly wants to test the deployed version, not local edits.
- `orvanta generate-metadata` — generate `.script.yaml` and `.lock` files for the script you modified.
...
On Orvanta, credentials and configuration are stored in resources and passed as parameters to main.
...
Import the orvanta client for platform interactions:
...
To accept a file from S3 as input to a script, type the parameter with `S3Object` (imported from `orvanta`):
...
def main(file: S3Object):
    content = orvanta.load_s3_file(file)
    # ...
```
...
content: bytes = orvanta.load_s3_file(s3object)

# Load file as stream reader
reader: BufferedReader = orvanta.load_s3_file_reader(s3object)

# Write file to S3
result: S3Object = orvanta.write_s3_file(
...
# Make an HTTP GET request to the Orvanta API.
# 
# Args:
...
# Make an HTTP POST request to the Orvanta API.
# 
# Args:
#     endpoint: API endpoint path
#     raise_for_status: Whether to raise an exception on HTTP errors
...
# 
# Args:
#     job_id: Job ID (defaults to current OV_JOB_ID)
# 
# Returns:
#     Root job ID
...
# List resources from Orvanta workspace.
# 
# Args:
...
# Get the Orvanta server version.
# 
# Returns:
#     Version string
def version()
...
# 
# s3_obj = S3Object(s3="/path/to/my_file.txt")
# with orvanta.load_s3_file_reader(s3object, s3_resource_path) as file_reader:
...
# 
# **Notes:**
# - This function must be executed within a Orvanta flow or flow preview.
# - The function checks for required environment variables (`OV_FLOW_JOB_ID`, `OV_FLOW_STEP_ID`) to ensure it is run in the appropriate context.
...
# - **v1 (OV_JOB_ID set, no @workflow)**: dispatches via HTTP API.
...
# Create a task that dispatches to a separate Orvanta script.
# 
# Usage::
# 
...
# Create a task that dispatches to a separate Orvanta flow.
# 
# Usage::
# 
