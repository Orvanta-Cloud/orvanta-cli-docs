...
An email trigger routes incoming emails to a script or flow. Each trigger reserves a local-part: emails sent to `<local_part>@<orvanta_email_domain>` are delivered to the configured runnable. Set `workspaced_local_part: true` to namespace it per workspace (the actual recipient becomes `<workspace_id>-<local_part>@…`); on Orvanta Cloud this is required.
...
```json
{ "s3": "orvanta_emails/<job_id>/attachments/<filename>" }
```

To read the bytes inside a script, use the orvanta SDK:

```ts
// TypeScript
import * as orvanta from "@orvanta/client"
const file = await orvanta.loadS3File(parsed_email.attachments[0].body)
```

```python
# Python
import orvanta
data = orvanta.load_s3_file(parsed_email["attachments"][0]["body"])
```
