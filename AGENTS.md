...
- `orvanta job list --script-path <path>` — recent runs of a specific script or flow
- `orvanta job list --failed --limit 20` — recent failures across the workspace
- `orvanta job get <id>` — status, timing, and (for flows) the step tree with sub-job IDs
- `orvanta job logs <id>` — stdout/stderr; for flows, aggregates every step's logs
- `orvanta job result <id>` — JSON result of a completed job
- `orvanta job cancel <id>` — stop a running or queued job

For flow failures, start with `orvanta job get <id>` to identify the failing step and its sub-job ID, then `orvanta job logs <sub-job-id>` to drill in.
